...
function J.GetAoeEnemyHeroLocation( bot, nCastRange, nRadius, nCount )
	...
	local nAoeCount = 0
	local nAoeTarget = nil
	if nAoe ~= nil then
	    nAoeCount = nAoe.count
	    nAoeTarget = nAoe.targetloc
	end
	...
	then
		...
		do
			if GetUnitToLocationDistance( enemy, nAoeTarget ) <= nRadius
		...
		then
			-- J.Utils.SetCachedVars(cacheKey, nAoeTarget)
			return nAoeTarget
		end
	end
