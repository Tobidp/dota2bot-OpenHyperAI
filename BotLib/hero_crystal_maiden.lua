...
function X.ConsiderQ()
	...
	if nCanHurtCreepsLocationAoE == nil
		or nCanHurtCreepsLocationAoETarget == nil
	...
	then
		if nCanHurtHeroLocationAoECount >= 2
			and GetUnitToLocationDistance( bot, nCanHurtHeroLocationAoETarget ) <= nCastRange + 50
		then
	...
	then
		...
		--进攻时对两名以上敌人使用
		if J.IsValid( nWeakestEnemyHeroInBonus )
			and nCanHurtHeroLocationAoECount >= 2
		...
		if 	J.IsValid( nEnemysWeakestLaneCreeps2 )
			and nCanHurtCreepsLocationAoECount >= 5
	...
	then
		...
		if nCanKillCreepsLocationAoECount >= 2
			and J.IsValid( nEnemysWeakestLaneCreeps1 )
		then
			return BOT_ACTION_DESIRE_HIGH, nCanKillCreepsLocationAoETarget
		end

		if nCanHurtCreepsLocationAoECount >= 4
			and J.IsValid( nEnemysWeakestLaneCreeps1 )
		then
			return BOT_ACTION_DESIRE_HIGH, nCanHurtCreepsLocationAoETarget
		end
