...
function X.ConsiderLaser()
    ...
    then
        ...
        then
            ...
            and #nEnemyLaneCreeps >= 2 and nLocationAoECount >= 2
            then
    ...
    then
        ...
        then
            ...
            and ((#nNeutralCreeps >= 2 and nLocationAoECount >= 2) or (#nNeutralCreeps == 1 and (nNeutralCreeps[1]:IsAncientCreep() or J.GetHP(nNeutralCreeps[1]) > 0.5)))
            then
...
function X.ConsiderMarchOfTheMachines()
	...
	then
        ...
        local nLocationAoE__ACount = 0
        local nLocationAoE__ATarget = nil
        if nLocationAoE__A ~= nil then
            nLocationAoE__ACount = nLocationAoE__A.count
            nLocationAoE__ATarget = nLocationAoE__A.targetloc
        end
        local nLocationAoE__E = bot:FindAoELocation(true, true, bot:GetLocation(), nDistance / 2, nRadius, 0, 0)
        local nLocationAoE__ECount = 0
        local nLocationAoE__ETarget = nil
        if nLocationAoE__E ~= nil then
            nLocationAoE__ECount = nLocationAoE__E.count
            nLocationAoE__ETarget = nLocationAoE__E.targetloc
        end
        if not J.IsCore(bot)
        then
            if nLocationAoE__ACount > 0
            and J.GetDistance(nLocationAoE__ATarget, nLocationAoE__ETarget) <= nDistance
            then
                return BOT_ACTION_DESIRE_HIGH, J.Site.GetXUnitsTowardsLocation(bot, nLocationAoE__ATarget, nCastRange)
            end
        end
        ...
        if nLocationAoE__ECount > 0
        then
            return BOT_ACTION_DESIRE_HIGH, J.Site.GetXUnitsTowardsLocation(bot, nLocationAoE__ETarget, nCastRange)
        end
	end
    ...
    then
        ...
        if nLocationAoECount >= 1
        then
            return BOT_ACTION_DESIRE_HIGH, J.Site.GetXUnitsTowardsLocation(bot, nLocationAoETarget, nCastRange)
        end
    end
    ...
    then
        ...
        then
            ...
            and ((#nNeutralCreeps >= 2 and nLocationAoECount >= 2)
...
function X.ConsiderDeployTurrets()
    ...
    then
        ...
        if nLocationAoECount >= 2
        then
            return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
        end
...
function X.ConsiderRearm()
    ...
    then
        ...
        if nLocationAoECount > 0
        and GetUnitToLocationDistance(bot, nLocationAoETarget) > 880
