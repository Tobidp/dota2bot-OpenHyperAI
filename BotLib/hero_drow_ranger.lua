...
function X.ConsiderE()
	...
	then
		...
		then
			...
			then
				...
				return BOT_ACTION_DESIRE_HIGH, nTargetLocation, "E-带线"..locationAoEHurtCount
			end
		end
	end
	...
	then
		...
		then
			...
			then
				nTargetLocation = locationAoETarget
				return BOT_ACTION_DESIRE_HIGH, nTargetLocation, "E-打钱"..locationAoECount
			end
		end
	end
...
function X.ConsiderW()
	...
	then
		...
		if locationAoECount >= 2
			or ( locationAoECount >= 1 and bot:GetHealth()/bot:GetMaxHealth() < 0.5 )
		then
			nTargetLocation = locationAoETarget
			return BOT_ACTION_DESIRE_HIGH, nTargetLocation
		end
