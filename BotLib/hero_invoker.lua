...
function X.ConsiderTornado()
    ...
    if J.IsInTeamFight(bot, 1200) then
        ...
        local nInRangeEnemy = J.GetEnemiesNearLoc(nLocationAoETarget, nRadius)
        local nInRangeAlly = J.GetAlliesNearLoc(nLocationAoETarget, nRadius)
    ...
    then
        ...
        and not J.IsLocationInChrono(nLocationAoETarget)
        and not J.IsLocationInBlackHole(nLocationAoETarget)
        then
...
function X.ConsiderEMP()
	...
	then
        ...
        and GetUnitToLocationDistance(bot, nLocationAoETarget) <= nCastRange
        then
            return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
		end
	end
...
function X.ConsiderCmToTarget(target, nDelay, nTravelDistance, nRadius)
    ...
    then
        ...
        if nLocationAoECount >= 2 then
            return BOT_ACTION_DESIRE_HIGH, X.AdjustCMLocation(nLocationAoETarget, target)
        end
...
function X.ConsiderChaosMeteor()
	...
	if J.IsLaning( bot ) then
        ...
        then
			...
			if locationAoEKillCount >= 3 then
				return BOT_ACTION_DESIRE_HIGH, X.AdjustCMLocation(locationAoEKillTarget, nil)
			end
		end
