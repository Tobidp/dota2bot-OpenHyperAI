...
function X.ConsiderQ()
	...
	local nCanKillHeroLocationAoECount = 0
	local nCanKillHeroLocationAoETarget = nil
	if nCanKillHeroLocationAoE ~= nil then
	    nCanKillHeroLocationAoECount = nCanKillHeroLocationAoE.count
	    nCanKillHeroLocationAoETarget = nCanKillHeroLocationAoE.targetloc
	end
	local nCanHurtHeroLocationAoE = bot:FindAoELocation( true, true, bot:GetLocation(), nCastRange, nRadius , 0, 0 )
	local nCanHurtHeroLocationAoECount = 0
	local nCanHurtHeroLocationAoETarget = nil
	if nCanHurtHeroLocationAoE ~= nil then
	    nCanHurtHeroLocationAoECount = nCanHurtHeroLocationAoE.count
	    nCanHurtHeroLocationAoETarget = nCanHurtHeroLocationAoE.targetloc
	end
	local nCanKillCreepsLocationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), nCastRange, nRadius, 0, nDamage )
	local nCanKillCreepsLocationAoECount = 0
	local nCanKillCreepsLocationAoETarget = nil
	if nCanKillCreepsLocationAoE ~= nil then
	    nCanKillCreepsLocationAoECount = nCanKillCreepsLocationAoE.count
	    nCanKillCreepsLocationAoETarget = nCanKillCreepsLocationAoE.targetloc
	end
	local nCanHurtCreepsLocationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), nCastRange, nRadius, 0, 0 )
	local nCanHurtCreepsLocationAoECount = 0
	local nCanHurtCreepsLocationAoETarget = nil
	if nCanHurtCreepsLocationAoE ~= nil then
	    nCanHurtCreepsLocationAoECount = nCanHurtCreepsLocationAoE.count
	    nCanHurtCreepsLocationAoETarget = nCanHurtCreepsLocationAoE.targetloc
	end


	if nCanHurtCreepsLocationAoE == nil
		or J.GetInLocLaneCreepCount( bot, 1600, nRadius, nCanHurtCreepsLocationAoETarget ) <= 2
	then
		 nCanHurtCreepsLocationAoECount = 0
	end


	if nCanKillHeroLocationAoECount ~= nil
		and nCanKillHeroLocationAoECount >= 1
	then
	...
	if bot:GetActiveMode() == BOT_MODE_LANING
		and nHP >= 0.4
	then
		if nCanHurtHeroLocationAoECount >= 2
			and J.IsValidHero( nWeakestEnemyHeroInBonus )
		then
			return BOT_ACTION_DESIRE_HIGH, nCanHurtHeroLocationAoETarget
		end
	end
	...
	then
		...
		local nCanHurtHeroLocationAoENearbyCount = 0
		local nCanHurtHeroLocationAoENearbyTarget = nil
		if nCanHurtHeroLocationAoENearby ~= nil then
		    nCanHurtHeroLocationAoENearbyCount = nCanHurtHeroLocationAoENearby.count
		    nCanHurtHeroLocationAoENearbyTarget = nCanHurtHeroLocationAoENearby.targetloc
		end
		if nCanHurtHeroLocationAoENearbyCount >= 1
			and J.IsValidHero( nWeakestEnemyHeroInBonus )
		then
			return BOT_ACTION_DESIRE_HIGH, nCanHurtHeroLocationAoENearbyTarget
		end
	end
	...
	then
		...
		if nCanHurtHeroLocationAoECount >= 2
			and GetUnitToLocationDistance( bot, nCanHurtHeroLocationAoETarget ) <= nCastRange
			and J.IsValidHero( nWeakestEnemyHeroInBonus )
		then
			return BOT_ACTION_DESIRE_HIGH, nCanHurtHeroLocationAoETarget
		end
		...
		if nCanHurtCreepsLocationAoECount >= 5
			...
			and DotaTime() >= 10 * 60
		then
			return BOT_ACTION_DESIRE_HIGH, nCanHurtCreepsLocationAoETarget
		end

		if nCanKillCreepsLocationAoECount >= 3
			...
			and DotaTime() >= 10 * 60
		then
			return BOT_ACTION_DESIRE_HIGH, nCanKillCreepsLocationAoETarget
		end
	end
	...
	then
		...
		then
			...
			if nCanHurtHeroLocationAoECount >= 3
				and GetUnitToLocationDistance( bot, nCanHurtHeroLocationAoETarget ) <= nCastRange
			then
				return BOT_ACTION_DESIRE_VERYHIGH, nCanHurtHeroLocationAoETarget
			end
        ...
        then
			...
			if J.CanKillTarget(enemyHero, nDamage, DAMAGE_TYPE_MAGICAL) then
				if bAoETalent then
					...
					if nLocationAoECount >= 2 then
						return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget, true
					else
	...
	do
		...
		then
			...
			then
				...
				local locationAoECount = 0
				local locationAoETarget = nil
				if locationAoE ~= nil then
				    locationAoECount = locationAoE.count
				    locationAoETarget = locationAoE.targetloc
				end
				if ( locationAoECount >= 2 ) then
					return BOT_ACTION_DESIRE_HIGH
				end
			end
		end
