...
function X.ConsiderAcidSpray()
	...
	then
		...
		then
			local realEnemyCount = J.GetEnemiesNearLoc(nLocationAoETarget, nRadius)
	...
	then
		...
		then
			...
			and ((#nNeutralCreeps >= 3 and nLocationAoECount >= 3)
				or (#nNeutralCreeps >= 2 and nLocationAoECount >= 2 and nNeutralCreeps[1]:IsAncientCreep()))
			then
				return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
			end
