...
function X.ConsiderW()
	...
	then
		...
		if ( locationAoECount >= 4 )
		then
			return BOT_ACTION_DESIRE_MODERATE, locationAoETarget
		end
	end
	...
	then
		...
		if ( locationAoECount >= 4 and #lanecreeps >= 4 )
		then
			return BOT_ACTION_DESIRE_MODERATE, locationAoETarget
		end
	end
	...
	then
		...
		then
			local nInvUnit = J.GetInvUnitInLocCount( bot, nCastRange, nRadius/2, locationAoETarget, false )
			if nInvUnit >= locationAoECount then
				return BOT_ACTION_DESIRE_MODERATE, locationAoETarget
			end
		end
	end
