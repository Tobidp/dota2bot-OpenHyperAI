...
function X.ConsiderQ()
	...
	local nCanHurtEnemyAoECount = 0
	local nCanHurtEnemyAoETarget = nil
	if nCanHurtEnemyAoE ~= nil then
	    nCanHurtEnemyAoECount = nCanHurtEnemyAoE.count
	    nCanHurtEnemyAoETarget = nCanHurtEnemyAoE.targetloc
	end
	if nCanHurtEnemyAoECount >= 3
	then
		nTargetLocation = nCanHurtEnemyAoETarget
	...
	then
		...
		then
			...
			then
				nTargetLocation = locationAoETarget
				return BOT_ACTION_DESIRE_HIGH, nTargetLocation, "Q打钱"..locationAoECount
			end
		end
	end
	...
	then
		...
		then
			...
			local locationAoEKillCount = 0
			local locationAoEKillTarget = nil
			if locationAoEKill ~= nil then
			    locationAoEKillCount = locationAoEKill.count
			    locationAoEKillTarget = locationAoEKill.targetloc
			end
			if locationAoEKillCount >= 3
			then
				nTargetLocation = locationAoEKillTarget
				return BOT_ACTION_DESIRE_HIGH, nTargetLocation, "Q带线补刀"..locationAoEKillCount
			end

			local locationAoEHurt = bot:FindAoELocation( true, false, bot:GetLocation(), nCastRange + 220, nRadius + 50, 0.8, 0 )
			local locationAoEHurtCount = 0
			local locationAoEHurtTarget = nil
			if locationAoEHurt ~= nil then
			    locationAoEHurtCount = locationAoEHurt.count
			    locationAoEHurtTarget = locationAoEHurt.targetloc
			end
			if locationAoEHurtCount >= 4
			then
				nTargetLocation = locationAoEHurtTarget
				return BOT_ACTION_DESIRE_HIGH, nTargetLocation, "Q带线清兵"..locationAoEHurtCount
			end
		end
	end
