...
function X.ConsiderQ()
	...
	then
		...
		if ( locationAoECount >= 2 ) then
			local target = J.GetVulnerableUnitNearLoc( bot, true, true, nCastRange, nRadius, locationAoETarget )
	...
	then
		...
		if ( locationAoECount >= 3 ) then
			local target = J.GetVulnerableUnitNearLoc( bot, false, true, nCastRange, nRadius, locationAoETarget )
			if target ~= nil then
				return BOT_ACTION_DESIRE_HIGH, target
			end
		end
...
function X.ConsiderW2()
	...
	if nCanKillHeroLocationAoECount >= 1
	then
