...
function X.ConsiderWaveform()
	...
	if J.IsPushing(bot) and bAttacking and not J.IsThereCoreNearby(1000) and nManaAfter > 0.35 then
        ...
        if J.CanBeAttacked(nEnemyCreeps[1]) and not J.IsRunning(nEnemyCreeps[1]) then
            ...
            if nLocationAoECount >= 4 then
                local nInRangeEnemy = J.GetEnemiesNearLoc(nLocationAoETarget, 1200)
	...
	if J.IsFarming(bot) and bAttacking and nManaAfter > 0.4 and bot:GetLevel() < 18 then
        ...
        then
            ...
            if (nLocationAoECount >= 3 or (nLocationAoECount >= 2 and nEnemyCreeps[1]:IsAncientCreep())) then
                return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
            end
        end
	end
    ...
    if nLocationAoECount >= 5 and #nEnemyHeroes == 0 and nManaAfter > 0.45 then
        return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
    end
