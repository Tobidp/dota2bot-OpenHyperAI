...
function X.SkillsComplement()
    ...
    then
        ...
        then
            ...
            then
                bot:Action_UseAbilityOnLocation(ColdFeet, nLocationAoETarget)
            else
...
function X.ConsiderIceVortex()
    ...
    then
        ...
        if nInRangeEnemy ~= nil and #nInRangeEnemy and GetUnitToLocationDistance(bot, nLocationAoETarget) <= nCastRange
        then
	...
	then
        ...
        and nLocationAoECount >= 4 and GetUnitToLocationDistance(bot, nLocationAoETarget) <= nCastRange
		then
			return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
		end
        ...
        if nLocationAoECount >= 2 and GetUnitToLocationDistance(bot, nLocationAoETarget) <= nCastRange
        then
            return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget
        end
	end
