		...
		if bot.trample_status ~= nil and type(bot.trample_status) == "table" then
			...
			elseif bot.trample_status[1] == 'farming' or bot.trample_status[1] == 'laning' then
                ...
                if nLocationAoE ~= nil then
					...
					if nLocationAoECount > 0 then
						DoTrample(nLocationAoETarget)
						return
					end
				else
					TrampleToBase()
	...
	then
		...
		elseif bot.onslaught_status[1] == 'farm' then
			...
			then
				...
				if ((#nCreeps >= 4 and nLocationAoECount >= 4))
				or (#nCreeps >= 2 and nLocationAoECount >= 2 and nCreeps[1]:IsAncientCreep())
				then
					bot:Action_MoveToLocation(nLocationAoETarget)
					return
				end
			end
		end
