...
X.ConsiderItemDesire["item_blink"] = function( hItem )
	...
	then
		...
		local nAOELocationCount = 0
		local nAOELocationTarget = nil
		if nAOELocation ~= nil then
		    nAOELocationCount = nAOELocation.count
		    nAOELocationTarget = nAOELocation.targetloc
		end
		local nEnemyLaneCreeps = bot:GetNearbyLaneCreeps(1600, true)
		local nInRangeEnemy = J.GetEnemiesNearLoc(nAOELocationTarget, 1600)

		if  nEnemyLaneCreeps ~= nil and #nEnemyLaneCreeps >= 4
		and nInRangeEnemy ~= nil and #nInRangeEnemy == 0
		and nAOELocationCount >= 4
		then
...
X.ConsiderItemDesire["item_guardian_greaves"] = function( hItem )
	...
	then
		...
		if nAOELocationCount >= 6
			and GetUnitToLocationDistance( bot, nAOELocationTarget ) <= 200
		then
			hEffectTarget = bot
...
X.ConsiderItemDesire["item_seer_stone"] = function(hItem)
	...
	then
		...
		do
			...
			then
				return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget, 'ground', nil
			end
		end
			...
			if J.IsGoingOnSomeone(allyHero) and J.IsAttacking(allyHero) then
				...
				then
					...
					if nLocationAoECount >= 2 then
						return BOT_ACTION_DESIRE_HIGH, nLocationAoETarget, 'point', nil
					else
...
function AbilityUsageThink()
	if RefreshBotHandle() then return end
	if bot:IsInvulnerable() or not bot:IsHero() or not bot:IsAlive() or not string.find(botName, "hero") or bot:IsIllusion() then return end
	if J.TryInterruptUnsafeChannel(bot) then return end
